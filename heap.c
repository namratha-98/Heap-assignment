...
static void swap(int32_t *data1, int32_t *data2){
    int32_t temp;
    temp = *data1;
    *data1 = *data2;
    *data2 = temp;
...
static void heapify (int32_t data[], uint32_t len, uint32_t parent){
...
Heap* heap_sort(Heap *heap){
    ...
    for (idx = heap->size; idx > 1; idx--){
        swap(&heap->data[idx], &heap->data[1]);
...
Heap*     heap_insert(Heap *heap, int32_t key){
    ...
    heapify(heap->data, heap->size, heap->size);
    return heap;
}
